...
export function isGameEnded(board) {
    return hasWinner(board).status || isBoardFull(board);
}

export function isGameTied(board) {
    return !hasWinner(board).status && isBoardFull(board)
}

export function isYourTurn(currentUser, player1, isPlayer1Turn) {
    return (currentUser === player1 && isPlayer1Turn) || (currentUser !== player1 && !isPlayer1Turn);
}
